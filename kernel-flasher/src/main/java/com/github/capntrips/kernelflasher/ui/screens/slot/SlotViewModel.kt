...
class SlotViewModel(
    ...
    init {
        try {
            ...
            Log.e(TAG, "Error initializing SlotViewModel", e)
            _error = e.message
        }
    }
    ...
    fun refresh(context: Context) {
        ...
        if (ramdisk.exists()) {
            when (Shell.cmd("$magiskboot cpio ramdisk.cpio test").exec().code) {
                ...
                1 -> _sha1 = Shell.cmd("$magiskboot cpio ramdisk.cpio sha1").exec().out.firstOrNull()
                    ...
                    if (kernel.exists()) {
                        Log.w(TAG, "Ignored invalid ramdisk because kernel exists (GKI Workaround)")
                        _sha1 = Shell.cmd("$magiskboot sha1 $boot").exec().out.firstOrNull()
                    } else {
                        Log.e(TAG, "Invalid ramdisk in boot.img")
                        if (inInit) _error = "Invalid ramdisk"
                    }
                }
            }
            ...
            _sha1 = Shell.cmd("$magiskboot sha1 $boot").exec().out.firstOrNull()
             ...
             Log.e(TAG, "Invalid boot.img, no ramdisk or kernel found")
             if (inInit) _error = "Invalid boot.img"
        }
        ...
        val partitionsToLoad = if (PartitionUtil.AvailablePartitions.isNotEmpty()) {
            PartitionUtil.AvailablePartitions
        } else {
            listOf("boot", "dtbo", "recovery", "vbmeta", "vendor_boot", "vendor_dlkm")
        }

        partitionsToLoad.forEach { partitionName ->
    ...
    // --- FUNGSI BARU UNTUK CARI MANUAL (ANTI-JSON ERROR) ---
    private fun safeFindPartition(name: String, suffix: String): ExtendedFile? {
        ...
        for (path in possiblePaths) {
            if (Shell.cmd("test -e $path").exec().isSuccess) {
                ...
                Log.d(TAG, "Backup fallback: Found $targetName at $resolved")
                return fileSystemManager.getFile(resolved)
            }
        }
        return null
    }
    ...
    fun saveLog(context: Context) {
        launch {
            ...
            val logName = when {
                navController.currentDestination?.route?.contains("ak3") == true -> "ak3"
                navController.currentDestination?.route?.endsWith("/backup") == true -> "backup"
                else -> "flash"
            }
            ...
            if (log.exists()) {
                ...
                log(context, "Failed to save $log")
            }
        }
    }
    ...
    private fun isPartitionMounted(partition: File): Boolean {
        ...
        if (partition.exists()) {
            val dmPath = Shell.cmd("readlink -f $partition").exec().out.getOrNull(0) ?: return false
    ...
    // --- FIX UTAMA DISINI: WRAP TRY-CATCH + FALLBACK MANUAL ---
    private fun backupPartitions(context: Context, destination: ExtendedFile): Partitions? {
        val partitions = HashMap<String, String>()
        
        val keys = _backupPartitions.keys.toList()
        
        for (partitionName in keys) {
            if (_backupPartitions[partitionName] == true) {
                var blockDevice: ExtendedFile? = null
                
                // 1. Coba pake Library dulu (PartitionUtil)
                try {
                    blockDevice = PartitionUtil.findPartitionBlockDevice(context, partitionName, slotSuffix)
                } catch (e: Exception) {
                    Log.w(TAG, "Library failed for $partitionName, trying manual fallback: ${e.message}")
                }

                // 2. Kalau null/error (misal karena JSON error), pake manual fallback
                if (blockDevice == null) {
                    blockDevice = safeFindPartition(partitionName, slotSuffix)
                }
                ...
                if (blockDevice != null) {
                    ...
                    addMessage("Skipping $partitionName (Not Found)")
                }
            }
        }
    ...
    private fun resetSlot() {
        val activeSlotSuffix = Shell.cmd("getprop ro.boot.slot_suffix").exec().out.getOrNull(0)
        if (activeSlotSuffix == null) return
